...
const handleResetPass = async () => {
  await forgetPass(ruleForm.email);
  ...
  <div v-if="!isResetPass" class="wrapper">
    <div class="inner">
      <el-form @submit.prevent ref="ruleFormRef" :rules="rules" :model="ruleForm">
        <h3 style="display: flex; align-items: center; flex-direction: column;"><img style="margin-bottom: 3px;"
            width="120px" src="/logo.png" />ĐĂNG NHẬP</h3>
        ...
        <div style="margin-top: 10px">
          <router-link to="/register" style="color: orangered">Đăng ký</router-link>
        </div>

        <button style="background-color: #fd6282; color: white" @click="handleLogin">
          <span>Đăng nhập</span>
        </button>
      </el-form>
    </div>
  ...
  <div v-if="isResetPass" class="wrapper" style="width: max-content">
    <div class="inner">
      <el-form @submit.prevent ref="ruleFormRef" :rules="rules" :model="ruleForm">
        ...
        <span>Nhập tài khoản email đã đăng ký tài khoản để nhận mật khẩu mới !</span>
        <div class="form-holder">
          <span class="lnr lnr-user"></span>
          <el-form-item prop="email">
            <input v-model="ruleForm.email" type="email" class="form-control" placeholder="Email" />

            <el-button style="background-color: orangered;" @click="handleResetPass">Khôi phục</el-button>
          </el-form-item>
        </div>
      </el-form>
    </div>
