...
const { productList, singleProduct, productSellTheMost } = storeToRefs(productStore);
  ...
  () => {
    selled.value = orderList.value.reduce((initValue, nextValue) => {
      if (nextValue.orderStatus === ORDER_STATUS.SUCCESS) {
        ...
        revenua.value = revenua.value + parseFloat(nextValue.totalMoney as string);
...
<template>
  <div class="product-container">
      ...
      <el-card
        ...
        >
          ...
          >
            ...
            <span
              >Giá :
              <b style="color: blue"> {{formatCurrency(productSellTheMost?.price) }}</b></span
            >
          </div>
        </div>
      </el-card>
    ...
    <ProductForm @close="handleCloseForm" v-if="isShowActionForm" />
  </div>
</template>
